...
class Heatmapper:
    ...
    def _colourised(self, img):
        ...
        arr = numpy.array(img)
        img = Image.open("assets/default.png")
        ...
        rtc =  LinearSegmentedColormap.from_list('from_image', colours)
        rgba_img = rtc(arr, bytes=True)
...
class PySideGreyHeatmapper(GreyHeatMapper):
    def __init__(self, point_diameter, point_strength):
        super(PySideGreyHeatmapper, self).__init__(point_diameter, point_strength)
