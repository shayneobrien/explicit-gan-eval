...
class Discriminator(nn.Module):
    def __init__(self, image_size, hidden_dim, output_dim):
        """
        Critic (not trained to classify). Input is an image (real or
        generated), output is the approximate Wasserstein Distance between
        z~P(G(z)) and real.
        """
...
class GAN(nn.Module):
    def __init__(self, image_size, hidden_dim, z_dim, output_dim=1):
        """
        Super class to contain both Discriminator / Critic (D)
        and Generator (G)
        """
...
class Trainer:
    ...
    def train(self, model, num_epochs, G_lr=5e-5, D_lr=5e-5, D_steps=5, clip=0.01):
        ...
        Inputs:
            ...
            D_lr: float, learning rate for discriminator's Adam optimizer
                (default 5e-5)
            D_steps: int, training step ratio for how often to train D compared
                to G (default 5)
            clip: float, bound for parameters [-c, c] to crudely ensure
                K-Lipschitz (default 0.01, (range [-0.01, 0.01]))
        Outputs:
            model: trained WGAN instance
        # Note that in the original paper, the authors use RMSprop for more
            stable training (no explanation given)
    ...
    ### GENERATE HISTOGRAMS INSTEAD OF IMAGES..... make sure agnostic!
